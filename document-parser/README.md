...
또한 이 단계엔 다음도 포함되지만 아직 손대지 않았습니다: 하드웨어가 페이지 이미지를 캡처해 서버로 올리는 흐름(1번 OCR 앞단), 오디오(wav) 실시간 전달 방식.

**점자 디스플레이(STM32) 연동은 시작됐습니다**: [`hardware/stm_pi_bridge/`](hardware/stm_pi_bridge/)에 STM32 보드 펌웨어(팀 작성, 검토만 함)와 그걸 이 서버(`document_parser.server`)에 연결하는 라즈베리파이용 브리지(`pi_bridge.py`, 새로 작성)가 있습니다. 자세한 내용은 그 폴더의 README 참고 — 프로토콜 로직은 자동화 테스트로 검증했지만, 실제 보드로 돌려본 적은 없습니다(이 컴퓨터엔 하드웨어가 없음).
...
| 5. 하드웨어 — 점자 디스플레이(STM32) | 진행 중 — 파이썬 브리지 작성·테스트 완료, 실기기 검증은 아직 |
