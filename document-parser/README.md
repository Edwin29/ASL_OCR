...
**하는 일**: 4번 서버가 만든 응답(점자 프레임 + 오디오)을 실제 점자 디스플레이·스피커가 달린 물리 기기로 전달하고, 반대로 기기의 버튼 입력을 서버로 전달합니다.

**점자 프레임 쪽은 HTTP로 결정되고 실제로 연결됐습니다**: [`server/http_server.py`](src/document_parser/server/http_server.py)가 [`server/wire.py`](src/document_parser/server/wire.py)의 `handle_wire_command()`를 HTTP로 감싼 실제 트랜스포트입니다 — 로컬 저장장치가 없는 호스트 기기(예: STM 보드에 블루투스로 연결된 PC)가 세션을 만들고(`POST /sessions`) 버튼마다 물어보는(`POST /sessions/<id>/command`) 구조입니다. [`hardware/stm_pi_bridge/`](hardware/stm_pi_bridge/)에서 이걸 STM32 보드의 시리얼 프로토콜과 실제로 잇는 브리지(`pi_bridge.py`)까지 만들어졌고, 실제 서버로 curl 왕복까지 검증했습니다(그 폴더 README 참고) — **다만 실제 STM 보드/블루투스로 테스트해본 적은 아직 없습니다**(이 컴퓨터엔 하드웨어가 없음).

**아직 손대지 않은 것**: 오디오(wav) 실시간 전달 방식(의도적으로 미룸, `audio_ref`는 여전히 서버 로컬 파일 경로만 담음), 하드웨어가 페이지 이미지를 캡처해 서버로 올리는 흐름(1번 OCR 앞단).
...
| 5. 하드웨어 — 오디오 재생 트리거 | 진행 중 — `pi_bridge.py`가 서버 응답의 오디오를 FRAME 줄과 같은 순간에 재생하도록 구현·검증(같은 컴퓨터에서만 실제 재생, 원격은 `audio_ref`가 아직 로컬 경로라 로그만) |
| 5. 하드웨어 — 오디오 바이트 전달(다른 컴퓨터 간), 이미지 캡처/업로드 | ❌ 미착수 |
