...
- **`test_client.py`** — **테스트 PC에서 실행하는 원스톱 스크립트**: 이미지 업로드(선택) → 완료 대기 → 데이터팩 선택 → 디스플레이 테스트 모드 진입까지 한 번에 처리합니다. 아래 "실행 방법"이 이 스크립트를 씁니다. `--port`가 주어지면(실제 보드) `device_flow.run_device_flow`를 그대로 호출해 선택 화면부터 실제 버튼으로 진행하고, `--port`가 없으면(콘솔 시뮬레이션) 기존 번호 입력 메뉴(`choose_book_id`)를 그대로 쓰되 `run_console_test_mode`가 `cl`(CONFIRM LONG)에서 `"selecting"`을 돌려주면 `main()`이 메뉴로 다시 루프백합니다 — 두 경로 모두 "선택 → 리딩 → (CONFIRM LONG) → 선택" 왕복 구조는 동일합니다.
- **`device_flow.py`** — **데이터팩 선택 화면과 리딩 세션을 하나의 루프로 묶는 오케스트레이터**, `test_client.py`의 실제-보드 경로가 그대로 쓰는 모듈입니다. 자세한 내용은 아래 "통합 선택+리딩 흐름" 절 참고.
...
**향후 스캔 모드에 대한 고려**: "새로 스캔" 같은 선택지는 아직 없습니다 — 카메라 캡처 루프와 전송 클라이언트는 별도 프로젝트인 `book-scanner`(다른 에이전트 소관)에 있고, 아직 이쪽과 연결된 적이 없습니다. 나중에 붙는다면 자연스러운 지점은 선택 화면입니다: 콘솔 쪽은 `choose_book_id`의 번호 메뉴에 항목 하나를 추가해 book_id 대신 book-scanner의 세션 루프로 분기하면 되고, 실제 보드 쪽은 `SelectionScreen`/`run_selecting_screen`에 책이 아닌 선택지를 하나 더 추가하면 됩니다. 지금은 그 자리만 비워둔 상태이고, 실제로 만들어 놓은 건 없습니다.
...
```
업로드 → 완료 대기 → `device_flow.run_device_flow` 진입까지 한 번에 진행됩니다. **`--port`가 주어지면 데이터팩 선택도 실제 상/하/확인 버튼으로 합니다** — 콘솔 번호 입력 단계 자체가 없습니다(위 "통합 선택+리딩 흐름" 절 참고). 방금 업로드한 책도 그냥 목록의 항목 하나로 UP/DOWN으로 찾아가면 됩니다.
...
**보드 없이 소프트웨어만 테스트** (`--port` 생략 → 콘솔에서 유니코드 점자로 시뮬레이션): 이 경로는 실제 버튼이 없으니 기존 번호 입력 메뉴를 그대로 씁니다. 리딩 화면에서 `cl`(CONFIRM LONG)을 입력하면 이 메뉴로 돌아옵니다 — 실제 보드의 CONFIRM LONG과 같은 왕복 구조입니다.
```bash
