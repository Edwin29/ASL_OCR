...
상태: **구현 및 실제 검증 완료 — p30 한정 후보 판정, 인간 검증 golden 적용**
작성일: 2026-08-30
승인일: 2026-08-30
완료일: 2026-08-30
선행 결과:
...
이 fixture는 Document Parser 개발 과정에서 사람이 직접 검증한 동일 원문 golden이다.
따라서 저장된 p30 본문·구조·점역 결과의 범위에서는 golden 대비 정확도와 variant 간 상대
비교에 사용할 수 있다.

다만 다음 제한은 유지한다.

- 이번 비교기가 산출하지 않은 CER/WER을 계산한 것처럼 표현하지 않는다.
- 세 반복 촬영의 결과를 다른 책·페이지·조명에 일반화하지 않는다.
- p30 golden의 지위를 p309 또는 신규 held-out 페이지에 자동 적용하지 않는다.

문제 번호 1~4, 문제 코드 4개, 각 문제의 선택지 존재, 페이지 번호 30도 수동 anchor로 함께
검사한다.
...
2. 사람이 검증한 p030 golden에 대한 동일 원문 정확도와 재현성
3. p30 밖의 다른 원문에 일반화하지 못한 범위
...
p030 fixture는 개발 과정에서 사람이 직접 검증한 golden으로 취급한다. 따라서 p30 fixture가
포함하는 본문·구조·점역 범위의 golden 대비 평가는 허용한다. 다만 별도 CER/WER 계산을 하지
않았으며, 이 지위를 다른 페이지로 확장하지 않는다.
