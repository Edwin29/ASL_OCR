...
/* USART1 interrupt is the sole producer and the main loop is the sole
 * consumer. Tokens are 16-bit so an out-of-band resynchronization marker can
 * never collide with a received byte. */
static volatile uint16_t bt_rx_ring[BT_RX_RING_SIZE];
static volatile uint16_t bt_rx_ring_head = 0U;
static volatile uint16_t bt_rx_ring_tail = 0U;
static volatile uint8_t bt_rx_drop_until_newline = 0U;
static volatile uint8_t bt_rx_resync_pending = 0U;
static volatile uint32_t bt_rx_overrun_errors = 0U;
static volatile uint32_t bt_rx_frame_errors = 0U;
static volatile uint32_t bt_rx_noise_errors = 0U;
static volatile uint32_t bt_rx_parity_errors = 0U;
static volatile uint32_t bt_rx_ring_overflows = 0U;
...
static void StartBluetoothInterruptReceive(void);
static void StopBluetoothInterruptReceive(void);
...
void BluetoothUartIrqHandler(void)
{
    uint32_t status = huart1.Instance->SR;
    uint32_t observed = status &
        (USART_SR_RXNE | USART_SR_ORE | USART_SR_FE | USART_SR_NE | USART_SR_PE);
    uint32_t errors;
    uint8_t ch;
    ...
    /* Reading SR followed by DR clears RXNE and the F4 UART error flags. */
    ch = (uint8_t)(huart1.Instance->DR & 0xFFU);
    errors = observed & (USART_SR_ORE | USART_SR_FE | USART_SR_NE | USART_SR_PE);
    if ((errors & USART_SR_ORE) != 0U)
        bt_rx_overrun_errors++;
    if ((errors & USART_SR_FE) != 0U)
        bt_rx_frame_errors++;
    if ((errors & USART_SR_NE) != 0U)
        bt_rx_noise_errors++;
    if ((errors & USART_SR_PE) != 0U)
        bt_rx_parity_errors++;
    BluetoothRxHandleByte(ch, errors);
}
...
static void StartBluetoothInterruptReceive(void)
{
    ...
    if ((status & (USART_SR_RXNE | USART_SR_ORE | USART_SR_FE | USART_SR_NE | USART_SR_PE)) != 0U)
    {
        volatile uint32_t discarded = huart1.Instance->DR;
        (void)discarded;
    }
