...
CONFIG=ROOT/"config"/"device-app.h3r-console-stm.toml"
LOG=ROOT/"logs"/"h3r-events.jsonl"
...
print(json.dumps({"type":"h3r_start","run_root":str(ROOT),"controls":"console","presenter":"stm_serial","stm_port":"COM9","initial_mode":"reading","physical_stm_nav_applied":False,"product_source_modified":False}),flush=True)
...
try: composition.application.run()
    ...
    print(json.dumps({"type":"h3r_stop","presentation_failures":composition.application.presentation_failures}),flush=True)
    log.close()
