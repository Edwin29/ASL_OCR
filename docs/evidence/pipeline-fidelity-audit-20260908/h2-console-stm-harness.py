...
ROOT=Path(r"C:\ASL_OCR_INTEGRATION_RUNTIME\demo-20260905\hardware-integration\h2-20260907-234639")
CONFIG=ROOT/"config"/"device-app.h2-console-stm.toml"
LOG=ROOT/"logs"/"h2-events.jsonl"
...
print(json.dumps({"type":"h2_start","run_root":str(ROOT),"controls":"console","presenter":"stm_serial","stm_port":"COM9","initial_mode":"reading","physical_stm_nav_applied":False,"product_source_modified":False}),flush=True)
...
try: composition.application.run()
    ...
    print(json.dumps({"type":"h2_stop","presentation_failures":composition.application.presentation_failures}),flush=True)
    log.close()
